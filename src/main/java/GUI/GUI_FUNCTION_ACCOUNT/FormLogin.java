...
public class FormLogin {
    ...
    public boolean checkMK(ArrayList<TaiKhoanDTO> list, String tk, String mk) throws NoSuchAlgorithmException {
        for (TaiKhoanDTO s : list) {
            if (s.getTaiKhoan().equals(tk)) {
                ...
                // QUY: khi lưu mật khẩu vào trong db -> list, lưu ở dạng HEX
                // nên khi so sánh, băm ra SHA-512 => lỗi
                ...
                // QUY: testing matKhau: 11112005
                System.out.println("String mk at arguments: " + mk);
                System.out.println("String mk in list: " + s.getMatKhau()); // hash: 3131313132303035

                String matKhau = sb.toString();

                // Testing
                System.out.println("Before compare to user input: " + matKhau);
    ...
    public void LoginFunction() throws IOException, NoSuchAlgorithmException {
        if (checkNull()) {
            String pass = new String(txtPass.getPassword());
            System.out.println("FormLogin -> LoginFunction -> before geAllList in TaiKhoanBUS");
